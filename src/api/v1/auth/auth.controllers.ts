...
export const signupRouteController = async (
    ...
    try {
        ...
        reply.status(200).type("application/json").send({
            success: true,
            message: "User registered successfully,OTP sent to provided email. Please verify.",
            data: {
...
export const verifyUserEmailRouteController = async (
    ...
    try {
        ...
        // Token payload
        const payload = { id: existingUser.dataValues.id, email: existingUser.dataValues.email };
        ...
        reply.status(400).send({ success: false, message: "Verification failed", error });
    }
};